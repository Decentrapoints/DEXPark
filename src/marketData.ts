...
export enum Tendency { Bear, Neutral, Bull, }
export async function updateCoinData(): Promise<CoinData> {
    let data: CoinData = {
        MANAETH: 0, MANABTC: 0,
        ETHUSDT: 0, BTCUSDT: 0,
        MANAUSDT: 0, LINKUSDT: 0,
        VETBUSD: 0, ALGOBUSD: 0,
        BNBUSDT: 0, XRPUSDT: 0,
        BCHUSDT: 0, LTCUSDT: 0,
        DOGEUSDT: 0,
    }
...
export enum StockDataTypes {
    BIGTITLE = 'bigtitle', BIGVALUE = 'bigvalue',
    TITLE = 'title', LABEL = 'label',
    VALUE = 'value', UNIT = 'unit',
    TINYVALUE = 'tinyvalue', TINYTITLE = 'tinytitle',
}
...
export class StockData extends Entity {
    constructor(type: StockDataTypes, text: string, 
        transform: TranformConstructorArgs,
        parent: Entity
    ){
        super();
        engine.addEntity(this);
        this.addComponent(new Transform(transform)); this.setParent(parent);
        let shape = new TextShape(text); shape.width = 10;
        switch (type) {
...
export function AddDataBoards(data: CoinData) {
    ...
    let board2Title = new StockData(
        ...
        '$USDT',
        {
           ...
           rotation: Quaternion.Euler(0, 180, 0),
           scale: new Vector3(16, 15.9, 0.01),
        }, board1
    );
    let board2Label1 = new StockData(
        StockDataTypes.LABEL,
        '/ LINK',
        { position: new Vector3(-0.4, 0.7, 0), } , board1
    );
    let board2Label2 = new StockData(
        StockDataTypes.LABEL,
        '/ VET',
        { position: new Vector3(-0.4, 0, 0), } , board1
    );
    let boar2Label3 = new StockData(
        StockDataTypes.LABEL,
        '/ ALGO', { position: new Vector3(-0.4, -0.7, 0), }, board1
    );
    let boar2Label4 = new StockData(
        StockDataTypes.LABEL,
        '/ XRP',
        { position: new Vector3(-0.4, -1.4, 0), }, board1
    );
    ...
    let board2Val4 = new StockData(
      ...
      { position: new Vector3(0.6, -1.4, 0), }, board1
    );
    ...
    let board1Title = new StockData(
      ...
      '$USDT',
      { position: new Vector3(1, 1, 0), }, board2
    );
  
    let board1Label1 = new StockData(
        StockDataTypes.LABEL,
        'MANA',
        { position: new Vector3(-0.7, 0.7, 0), }, board2
    );
    let board1Label2 = new StockData(
        StockDataTypes.LABEL,
        'ETH',
        { position: new Vector3(-0.7, -0.3, 0), }, board2
    );
    let board1Label3 = new StockData(
        StockDataTypes.LABEL,
        'BTC',
        { position: new Vector3(-0.68, -1.3, 0), }, board2
    );
    ...
    let board3Title = new StockData(
        StockDataTypes.TITLE,
        '$USDT',
        { position: new Vector3(1, 1.5, 0), }, board3
    );
  
    let board3Label1 = new StockData(
        StockDataTypes.LABEL,
        'BNB',
        { position: new Vector3(-0.7, 0.7, 0), } , board3
    );
    let board3Label2 = new StockData(
        StockDataTypes.LABEL,
        'BCH',
        { position: new Vector3(-0.7, 0, 0), } , board3
    );
    ...
    let boar3Label4 = new StockData(
        StockDataTypes.LABEL,
        'DOGE',
        { position: new Vector3(5, -1.4, 0), } , board3
    );
